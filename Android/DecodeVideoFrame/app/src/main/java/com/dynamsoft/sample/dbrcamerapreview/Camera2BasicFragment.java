...
public class Camera2BasicFragment extends Fragment
    ...
    private void closeCamera() {
        try {
            ...
            if (ifStartDecode) {
                ifStartDecode = false;
                mMainActivity.getMainBarcdoeReader().stopFrameDecoding();
            }
     ...
     */
    private boolean ifStartDecode = false;
    private void startBackgroundThread() {
        ...
        mBackgroundHandler2 = new Handler(mBackgroundThread2.getLooper()){
            ...
            try {
                if (!ifStartDecode) {
                    ifStartDecode = true;
                    FrameDecodingParameters parameters = mMainActivity.getMainBarcdoeReader().initFrameDecodingParameters();
                    parameters.maxQueueLength = 10;
                    parameters.maxResultQueueLength = 20;
                    parameters.width = imageData.mWidth;
                    parameters.height = imageData.mHeight;
                    parameters.stride = imageData.mStride;
                    parameters.imagePixelFormat = EnumImagePixelFormat.IPF_NV21;
                    mMainActivity.getMainBarcdoeReader().setTextResultCallback(new TextResultCallback() {
                        @Override
                        public void textResultCallback(int i, TextResult[] results, Object o) {
                    ...
                    mMainActivity.getMainBarcdoeReader().startFrameDecodingEx(parameters, "");
                }
                mMainActivity.getMainBarcdoeReader().appendFrame(imageData.mBytes);
            } catch (Exception e) {
                e.printStackTrace();
                Message message = mHandler.obtainMessage();
                message.what = 0x02;
