...
public class Camera2BasicFragment extends Fragment
    ...
    @SuppressLint("HandlerLeak")
      private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0x02:
                    showResult((TextResult[])msg.obj);
                    break;
            }
        }
    };
    private void showResult(TextResult[] results) {
        View dialogView = LayoutInflater.from(getActivity()).inflate(R.layout.result_dialog, null);
        TextView tvFormat = dialogView.findViewById(R.id.tv_result_format);
        final TextView tvText = dialogView.findViewById(R.id.tv_result_text);
        TextView tvDecodeTime = dialogView.findViewById(R.id.tv_result_decode_time);
        RelativeLayout viewCopy = dialogView.findViewById(R.id.view_text);
        tvFormat.setText(results[0].barcodeFormatString);
        tvText.setText(results[0].barcodeText);
        tvDecodeTime.setText("Total time spent: " + timeMillis);
        viewCopy.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String copyStr = tvText.getText().toString();
                ClipboardManager clipboardManager = (ClipboardManager) getActivity().getSystemService(CLIPBOARD_SERVICE);
                ClipData clipData = ClipData.newPlainText("", copyStr);
                clipboardManager.setPrimaryClip(clipData);
                Toast.makeText(getActivity(), "Text has been copied to clipboard.", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        public void onImageAvailable(ImageReader reader) {
            if (reader != null) {
                ...
                if (mImage != null) {
                    ...
                    if(mBackgroundHandler2!=null &&!bDecoding && !ifShowResult) {
                        bDecoding = true;
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                PublicRuntimeSettings settings = mMainActivity.getMainBarcdoeReader().getRuntimeSettings();
                settings.region = handleRegion(mQrCropRect.left, mQrCropRect.top, mQrCropRect.width(), previewScaleX, previewScaleY, outputPreviewSize.getHeight(), outputPreviewSize.getWidth(), dependOnWid);
                mMainActivity.getMainBarcdoeReader().updateRuntimeSettings(settings);
                return;
            }
    ...
    private RegionDefinition handleRegion(int left, int top, int width, float previewScaleX, float previewScaleY, int srcBitmapHeight, int srcBitmapWidth, boolean dependOnWid) {
        RegionDefinition newRegion = new RegionDefinition();
        newRegion.regionMeasuredByPercentage = 0;
        if (dependOnWid) {
            newRegion.regionBottom = (int)(srcBitmapHeight - left / previewScaleX);
            newRegion.regionLeft = (int)((top + Math.abs(previewScaleX - previewScaleY) * srcBitmapWidth / 2) / previewScaleX);
            newRegion.regionTop= (int)(newRegion.regionBottom - width / previewScaleX);
            newRegion.regionRight = (int)(newRegion.regionLeft + width / previewScaleX);
        } else {
            newRegion.regionBottom = (int)(srcBitmapHeight - left / previewScaleY);
            newRegion.regionLeft = (int)(top / previewScaleY);
            newRegion.regionTop= (int)(newRegion.regionBottom - width / previewScaleY);
            newRegion.regionRight = (int)(newRegion.regionLeft + width / previewScaleY);
        }
        return newRegion;
    }
    /**
                    ...
                    new CameraCaptureSession.StateCallback() {
                        ...
                        public void onConfigured(@NonNull CameraCaptureSession cameraCaptureSession) {
                            ...
                            try {
                                ...
                                int[] a = mCameraCharacteristics.get(CameraCharacteristics.CONTROL_AE_AVAILABLE_ANTIBANDING_MODES);
                                mPreviewRequestBuilder.set(CaptureRequest.CONTROL_MODE, CaptureRequest.CONTROL_MODE_AUTO);
                                mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AWB_MODE, CameraMetadata.CONTROL_AWB_MODE_WARM_FLUORESCENT);
