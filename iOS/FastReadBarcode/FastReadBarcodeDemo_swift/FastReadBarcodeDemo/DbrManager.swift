...
class  DbrManager: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate,DBRServerLicenseVerificationDelegate {
    ...
    init(license:String)
    {
        ...
        //Best Speed settings
        barcodeReader.initRuntimeSettings(with: "{\"Version\":\"3.0\",\"ImageParameterContentArray\":[{\"Name\":\"BestSpeed\",\"FormatSpecificationNameArray\":[\"FP_1\"],\"ExpectedBarcodesCount\":1,\"MaxAlgorithmThreadCount\":4,\"BarcodeFormatIds\":[\"BF_QR_CODE\"],\"LocalizationModes\":[\"LM_SCAN_DIRECTLY(,2)\"],\"ScaleUpModes\":[\"SUM_SKIP\"],\"DeblurLevel\":0}],\"FormatSpecificationArray\":[{\"Name\":\"FP_1\",\"BarcodeFormatIds\":[\"BF_QR_CODE\"],\"MirrorMode\":\"MM_NORMAL\"}]}", conflictMode: EnumConflictMode.overwrite, error: &err)
        
        settings = try! barcodeReader.getRuntimeSettings()
        settings.barcodeFormatIds = Int(EnumBarcodeFormat.QRCODE.rawValue) | Int(EnumBarcodeFormat.ONED.rawValue)
        settings.deblurLevel = 0;
        settings.localizationModes = [16, 2, 4, 8, 0, 0, 0, 0];
    ...
    func parametersInit()
    {
        ...
        barcodeFormat = Int(EnumBarcodeFormat.QRCODE.rawValue)
        barcodeFormat2 = 0
