...
- (void) configCellsBackground{
    ...
    [dotCodeCell setBackgroundView:bgColorView];
}
...
-(void) viewWillDisappear:(BOOL)animated {
    if ([self.navigationController.viewControllers indexOfObject:self] == NSNotFound) {
        ...
        long allBarcodeFormatTypeInvert = ~EnumBarcodeFormatALL;
        long allBarcodeFormat2TypeInvert = ~(EnumBarcodeFormat2POSTALCODE | EnumBarcodeFormat2NONSTANDARDBARCODE | EnumBarcodeFormat2DOTCODE);
        if(mainView != nil && mainView.dbrManager != nil){
            [mainView.dbrManager setBarcodeFormat:(mainView.dbrManager.barcodeFormat & allBarcodeFormatTypeInvert) barcodeFormat2:(mainView.dbrManager.barcodeFormat2 & allBarcodeFormat2TypeInvert)];
